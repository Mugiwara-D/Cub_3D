/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   raycasting.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: xacharle <xacharle@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/02 19:39:13 by xacharle          #+#    #+#             */
/*   Updated: 2024/04/04 15:46:36 by xacharle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static void	ft_ray_initial_calculations(t_game *game, t_ray *r, float v)
{
	r->dx = cos(v);
	r->dy = -sin(v);
	r->sx = get_sign(r->dx);
	r->sy = get_sign(r->dy);
	r->verthit_x = (int) game->px;
	if (r->sx > 0)
		r->verthit_x += 1.0f;
	r->horhit_y = (int) game->py;
	if (r->sy > 0)
		r->horhit_y += 1.0f;
	r->hor_w = 0;
	r->verthit_y = 0;
	r->horhit_x = 0;
	r->vert_w = 0;
}

static void	ft_ray_next_step_calculation(t_game *game, t_ray *r)
{
	if (r->sx != 0)
	{
		r->verthit_y = game->py + r->dy / r->dx * (r->verthit_x - game->px);
		r->vert_dist = sqrt(pow(game->px - r->verthit_x, 2.0)
				+ pow(game->py - r->verthit_y, 2.0));
		r->vert_w = r->verthit_y - (int) r->verthit_y;
		if (r->sx > 0)
			r->vert_w = 1 - r->vert_w;
	}
	else
		r->vert_dist = INFINITY;
	if (r->sy != 0)
	{
		r->horhit_x = game->px + r->dx / r->dy * (r->horhit_y - game->py);
		r->hor_dist = sqrt(pow(game->px - r->horhit_x, 2.0)
				+ pow(game->py - r->horhit_y, 2.0));
		r->hor_w = r->horhit_x - (int) r->horhit_x;
		if (r->sy < 0)
			r->hor_w = 1 - r->hor_w;
	}
	else
		r->hor_dist = INFINITY;
}

static float	ft_save_color(t_game *game, float dist, int color_idx, float w)
{
	game->text_idx = color_idx;
	game->text_w = w;
	return (dist);
}

float	ft_ray(t_game *game, float v)
{
	t_ray	r;

	ft_ray_initial_calculations(game, &r, v);
	while (1)
	{
		ft_ray_next_step_calculation(game, &r);
		if (r.vert_dist < r.hor_dist)
		{
			if (game->rectmap[(int)r.verthit_y][(int)r.verthit_x + (r.sx - 1) / 2] == '1')
				return (ft_save_color(game, r.vert_dist, r.sx + 1, r.vert_w));
			else
				r.verthit_x += r.sx;
		}
		else
		{
			if (game->rectmap[(int)r.horhit_y + (r.sy - 1) / 2][(int)r.horhit_x] == '1')
				return (ft_save_color(game, r.hor_dist, r.sy + 2, r.hor_w));
			else
				r.horhit_y += r.sy;
		}
	}
}

void	ft_ray_casting(t_game *game)
{
	int		x;
	float	dv;
	float	v;

	v = game->angle - FOV / 2;
	dv = FOV / (WIDTH - 1);
	x = -1;
	while (++x < WIDTH)
	{
		ft_line(game, x, ft_ray(game, v) * cos(game->angle - v));
		v += dv;
	}
}
