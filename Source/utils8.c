...
static void	ft_ray_next_step_calculation(t_game *game, t_ray *r)
{
	if (r->sx != 0)
	{
		r->vert_y = game->py + r->dy / r->dx * (r->vert_x - game->px);
		r->vert_dist = sqrt(pow(game->px - r->vert_x, 2.0)
				+ pow(game->py - r->vert_y, 2.0));
	...
	if (r->sy != 0)
	{
		r->hor_x = game->px + r->dx / r->dy * (r->hor_y - game->py);
		r->hor_dist = sqrt(pow(game->px - r->hor_x, 2.0)
				+ pow(game->py - r->hor_y, 2.0));
...
float	ft_ray(t_game *game, float v)
{
	...
	while (1)
	{
		...
		{
			if (game->rectmap[(int)r.vert_y][(int)r.vert_x + (r.sx - 1) / 2] == '1')
		...
		{
			if (game->rectmap[(int)r.hor_y + (r.sy - 1) / 2][(int)r.hor_x] == '1')
